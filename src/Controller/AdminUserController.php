...
class AdminUserController extends Controller
{
    ...
    public function showAction(Request $request)
    {
        ...
        $query = $this->getDoctrine()->getRepository('AppBundle:User')->findFilteredByClubQuery($club, $searchQuery);
        ...
        return $this->render('user/show_users.html.twig', array(
            'users' => $users,
    ...
    public function showSpecificAction(User $user)
    {
        ...
        return $this->render('user/user_specific.html.twig', array(
            'user' => $user,
        ));
    }
    ...
    public function changeUserTypeAction(Request $request)
    {
        ...
        if (
            !$this->get('user.roles')->isValidRole($userRole) ||
            $user === $this->getUser()
        ) {
    ...
    {
        //Remove participation from all courses this and future semesters
        $participants = $this->getDoctrine()->getRepository('AppBundle:Participant')->findByUserThisAndLaterSemesters($user);
        $manager = $this->getDoctrine()->getManager();
    ...
    private function removeCurrentTutors($user)
    {
        ...
        $manager = $this->getDoctrine()->getManager();
        $tutors = $this->getDoctrine()->getRepository('AppBundle:Tutor')->findByUserThisAndLaterSemesters($user);
