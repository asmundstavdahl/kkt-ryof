...
class CourseQueueController extends Controller
{
    ...
    {
        $queueEntities = $this->getDoctrine()->getRepository('AppBundle:CourseQueueEntity')->findByCourse($course);
    ...
    public function enqueueAction(Request $request, Course $course)
    {
        ...
        if ($this->isGranted('ROLE_PARENT')) {
            ...
            if (null === $childId || null === $child = $this->getDoctrine()->getRepository('AppBundle:Child')->find($childId)) {
                throw new BadRequestHttpException();
            }
            $alreadyInQueue = null !== $this->getDoctrine()->getRepository('AppBundle:CourseQueueEntity')->findByCourseAndChild($course, $child);
            $alreadyInCourse = null !== $this->getDoctrine()->getRepository('AppBundle:Participant')->findByCourseAndChild($course, $child);

            $queueEntity->setChild($child);
        } else {
            $alreadyInQueue = null !== $this->getDoctrine()->getRepository('AppBundle:CourseQueueEntity')->findOneByUserAndCourse($this->getUser(), $course);
            $alreadyInCourse = null !== $this->getDoctrine()->getRepository('AppBundle:Participant')->findOneByUserAndCourse($this->getUser(), $course);
        }
